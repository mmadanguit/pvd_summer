...
fill <- function(df, period){
  ...
  df <- df %>% group_by(TRACT_latlng) %>% 
          complete(nesting(TRACT_latlng), DATE = period, fill = list(TRIPS=0))
  return(df)
}

file <- "events2018.csv"

pickups <- roundLatLng(file) %>%
  splitTimeCol() %>%
  preClean() %>%
  mapToTract() %>%
  postClean()
...
pickupsSummary <- pickups %>% 
  group_by(TRACT_latlng) %>% 
  group_by(DATE, .add=TRUE) %>% 
  dplyr::summarize(DATE, TRIPS=n(), .groups="keep") %>% # for each day in each tract
  distinct() %>%
  fill(period)
